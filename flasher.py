...
def list_disks():
    """List physical disks on Windows. Identifies system drive and filters USB/SD drives."""
    try:
        # Get C: drive disk number
        cmd_c = ["powershell", "-Command", "Get-Partition -DriveLetter C | Get-Disk | Select-Object -ExpandProperty Number"]
    ...
    try:
        ...
        cmd_disks = ["powershell", "-Command", "Get-Disk | Select-Object Number, FriendlyName, Size, BusType | ConvertTo-Json"]
        ...
        # Handle single disk returned as dict instead of list
        ...
        for d in disks_data:
            ...
            # Format size to human readable (GB)
...
def fetch_board_images(board_id):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            for link in links:
                # We only want .img.xz files and skip signatures/torrents
                if link.endswith('.img.xz') and not link.endswith('.asc') and not link.endswith('.sha'):
                    # Filename example: Armbian_25.5.1_Orangepizero3_bookworm_current_6.12.23_minimal.img.xz
                    ...
                    # Parse kernel version if available
...
def decompress_xz(xz_path, img_path, progress_callback=None):
    ...
    # We track progress by reading the compressed file in chunks and decompressing
...
def _build_setup_script(hostname, username, password, wifi_ssid, wifi_password, ssh_key=None, enable_ssh=False, preload_apps=None):
    """Build a standalone bash setup script that applies all configuration."""
    lines = [
        "#!/bin/bash",
        "# Auto-configuration script generated by Creamsicle: OS Buddy",
        "# Run as root: sudo bash setup_orangepi.sh",
        "set -e",
        "",
    ]
    ...
    if wifi_ssid:
        lines.extend([
            ...
            "# Try NetworkManager first (used by most modern images)",
            f"if command -v nmcli >/dev/null 2>&1; then",
            f"    nmcli device wifi connect '{wifi_ssid}' password '{wifi_password}' || true",
            "else",
            "    # Fallback to wpa_supplicant config",
...
def customize_image(img_path, hostname, username, password, wifi_ssid, wifi_password, ssh_key=None, enable_ssh=False, preload_apps=None):
    """Inject configuration files into the boot partition of the image.

    Returns (success, setup_script_path_or_none, diagnostics_list).
    - If FAT boot partition found: writes config directly, returns (True, None, diag)
    - If no FAT partition: saves setup script to disk, returns (False, script_path, diag)
    """
    setup_script = _build_setup_script(
        hostname, username, password, wifi_ssid, wifi_password,
        ssh_key, enable_ssh, preload_apps
    )

    # Build Armbian-style first-run config (FR_* variables + shell commands)
    ...
    def try_write_config(root_dir):
        """Write config files into an opened FAT volume root directory."""
        ...
        # Also write the full setup script to the boot partition
        f_setup = root_dir.create("setup_orangepi.sh")
        f_setup.write(setup_script.encode('utf-8'))
        f_setup.close()

    def is_fat_volume(obj):
        return obj is not None and not isinstance(obj, str) and hasattr(obj, 'create')

    # Strategy 1: Auto-detect
    try:
    ...
    # Strategy 2: Try each partition index explicitly
    for i in range(4):
        try:
    ...
    # No FAT partition found — save setup script to local file for post-boot use
    log("No FAT boot partition found. Saving setup script for post-boot SSH application.")
    script_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "temp")
    os.makedirs(script_dir, exist_ok=True)
    script_path = os.path.join(script_dir, "setup_orangepi.sh")
    with open(script_path, 'w', newline='\n') as f:
        f.write(setup_script)
    log(f"Setup script saved to: {script_path}")

    return False, script_path, diag

def flash_to_drive(img_path, disk_number, progress_callback=None):
    """Write the image file to the physical drive on Windows."""
    # 1. Clean the disk using diskpart to release all OS locks
    ...
    # 2. Write image file to the raw physical drive using Win32 API
    try:
        physical_drive = f"\\\\.\\PhysicalDrive{disk_number}"
        ...
        # Open physical drive via CreateFileW for raw write access
        ...
        if handle == INVALID_HANDLE_VALUE:
            ...
            if progress_callback:
                progress_callback("error", f"Cannot open {physical_drive} (Windows error {err}). Is the drive in use?")
            return False

        try:
            # Wrap the Win32 handle in a Python file descriptor
            fd = msvcrt.open_osfhandle(handle, os.O_WRONLY)
            # Align read buffer to 512-byte sectors for FILE_FLAG_NO_BUFFERING
            CHUNK_SIZE = 4 * 1024 * 1024  # 4 MB (multiple of 512)
