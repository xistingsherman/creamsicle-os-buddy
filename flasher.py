...
def _build_user_script(username, password):
    ...
    Must complete and the user must log back in before running setup_config.sh.
    """
    lines = [
        "#!/bin/bash",
...
def _build_config_script(hostname, username, wifi_ssid, wifi_password, ssh_key=None, enable_ssh=False, preload_apps=None):
    """Build Step 2 script: apply all system configuration.
    Run as root after logging back in as the new user created by setup_user.sh.
    """
    lines = [
        "#!/bin/bash",
...
def customize_image(img_path, hostname, username, password, wifi_ssid, wifi_password, ssh_key=None, enable_ssh=False, preload_apps=None):
    ...
    Returns (success, script_paths_or_none, diagnostics_list).
    - If FAT boot partition found: writes config directly, returns (True, None, diag)
    - If no FAT partition: saves both scripts to disk, returns (False, [user_path, config_path], diag)
    """
    ...
    # No FAT partition found — save both scripts locally for post-boot SSH application
